...
tmux set -g status-left "#[fg=${TMUX_COLOR_DARK_GRAY},bg=${TMUX_STATUS_COLOR},bold] #S "
tmux set -g status-right "#[fg=${TMUX_STATUS_COLOR}]#{?client_prefix,P,} #(~/.config/tmux/network.sh) #(~/.config/tmux/memory.sh) #(~/.config/tmux/cpu.sh) #[fg=${TMUX_COLOR_DARK_GRAY},bg=${TMUX_STATUS_COLOR},bold] #h "
tmux setw -g window-status-format "#[fg=${TMUX_COLOR_LIGHT_GRAY}] #I:#W "
tmux setw -g window-status-current-format "#[fg=${TMUX_COLOR_WHITE},bg=${TMUX_COLOR_LIGHT_GRAY}] #{?window_zoomed_flag,,#{?pane_synchronized,,#I}}:#W "
