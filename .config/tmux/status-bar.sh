...
tmux set -g status-left "#[fg=${TMUX_COLOR_DARK_GRAY},bg=${TMUX_STATUS_COLOR}]#{?client_prefix,, }#S "
tmux set -g status-right "#[fg=${TMUX_STATUS_COLOR}]#(~/.config/tmux/network.sh) #(~/.config/tmux/memory.sh) #(~/.config/tmux/cpu.sh) #[fg=${TMUX_COLOR_DARK_GRAY},bg=${TMUX_STATUS_COLOR}] #h "
